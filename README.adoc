...
| URL
| ``
| URL of REST resource provided by CouchDB (or other compatible service) where the ratings should be submitted to, e.g. `http://couchdb:5984/ratings`. Unless left empty, the VLO will try to connect on startup and fail if unavailable.
...
| VLO_DOCKER_RATING_COUCHDB_INIT
| Yes
| Boolean
| `false`
| Set to `true` if the container should try to initialise the configured CouchDB service on startup. Note that this will not destroy or affect in any way an already initialised instance.
